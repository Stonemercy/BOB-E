...
module.exports = {
	...
	async execute(interaction) {
		...
		const targetUser = await Users.findOne({ where: { user_id: target.id, guild_id: interaction.guildId } });
		...
		if (!currentGuild.shop_channel_id) {
			...
			return interaction.reply(`You have ${currentGuild.shop_currency} ${targetUser.balance}`);
		}
		else if (interaction.options.getUser('user')) {
			return interaction.reply(`<@${target.id}> has ${currentGuild.shop_currency} ${targetUser.balance}`);
		}
	},
};